  ...
  switch (type) {
    case UPDATE_EMAIL: {
      try {
        ...
        yield call(api.updateEmail, payload.password, payload.newEmail);
    ...
    case UPDATE_PROFILE: {
      try {
        ...
        if (email && password && email !== state.profile.email) {
          yield call(api.updateEmail, password, email);
        }

        if (avatarFile || bannerFile) {
          const bannerURL = bannerFile ? yield call(api.storeImage, state.auth.id, 'banner', bannerFile) : payload.updates.banner;
          const avatarURL = avatarFile ? yield call(api.storeImage, state.auth.id, 'avatar', avatarFile) : payload.updates.avatar;
          const updates = { ...payload.updates, avatar: avatarURL, banner: bannerURL };

          yield call(api.updateProfile, state.auth.id, updates);
          yield put(updateProfileSuccess(updates));
        } else {
          yield call(api.updateProfile, state.auth.id, payload.updates);
